...
The following charts show estimate Victoria-wide ATSI employment statistics and compare them with broader population values. Please note, census-based employment statistics will not reconcile with official labour statistics. For more information [please read the data quality statement](#data-quality). 
...
hc_emp_pop <- headline_data %>% 
  ...
  hc_title(
    text = "Employment to</br>population ratio"
  ) %>% 
  hc_tooltip(
    formatter = JS(
      "function () {
...
hchart(
  ...
  hc_subtitle(
    text = "Regional ATSI unemployment rates by distance from Melbourne and smoothed trend line"
  ) %>% 
  hc_tooltip(
    formatter = JS(
      "function () {
...
hchart(
  ...
  hc_subtitle(
    text = "Regional ATSI participation rates by distance from Melbourne and smoothed trend line"
  ) %>% 
  hc_tooltip(
    formatter = JS(
      "function () {
...
[Full data is available for download here](https://github.com/djsir-data/atsilabourforce/raw/main/data/Vic%20ATSI%20employment.xlsx). Alternatively, you can manually explore the data in the following table.
...
write_xlsx(out_data, "data/Vic ATSI employment.xlsx")
...
The information contained in this report attempts to bridge two knowledge gaps; ATSI employment rates since census and fine geographical disaggregation of ATSI employment. These new features may introduce the following errors:

-   **Small regions with few ATSI people may report anomalous results**\
    For example, if a given region has few ATSI people but they are all employed, the region would report an unemployment rate of zero. Though technically correct for the snapshot of the region during census, this result does not represent the underlying conditions or likelihood of employment for ATSI people living here.

-   **Movements in employment since Census don't capture ATSI-specific changes**\
    While census provides a reasonably accurate picture of ATSI employment conditions, the [Australian Bureau of Statistics time series employment publication](https://www.abs.gov.au/statistics/labour/employment-and-unemployment/labour-force-australia-detailed) does not provide any detailed ATSI information. This means that if ATSI employment conditions improved or deteriorated at a different rate to the general public, this report will not reflect the additional changes.
